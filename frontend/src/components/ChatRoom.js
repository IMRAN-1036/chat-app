...
import { EMOJI_LIST, QUICK_REACTIONS, TIME_CONSTANTS, UI_CONSTANTS, ERROR_MESSAGES, LOCAL_STORAGE_KEYS } from '../constants';
import { sanitizeAndValidateMessage, validateUsername } from '../utils/validation';
import { handleApiError, retryWithBackoff } from '../utils/errorHandler';
import { debounce, messageRateLimiter, throttle } from '../utils/rateLimit';
...
function timeAgo(date) {
  ...
  if (s < TIME_CONSTANTS.SECONDS_PER_MINUTE) return s < 10 ? "now" : `${s}s`;
  const m = Math.floor(s / TIME_CONSTANTS.SECONDS_PER_MINUTE);
  if (m < 60) return `${m}m`;
